...
class BasicLayer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, token_dims=64, downsample_ratios=4, kernel_size=7, RC_heads=1, NC_heads=6, dilations=[1, 2, 3, 4],
                ...
                qk_scale=None, drop=0, attn_drop=0., norm_layer=nn.LayerNorm, window_size=(14, 14), use_checkpoint=False, interval=3):
        super().__init__()
        ...
        self.NC = nn.ModuleList([
            NormalCell(token_dims, NC_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                       ...
                       img_size=img_size // downsample_ratios, window_size=window_size if (i + 1) % interval != 0 else full_image_size, 
                       window=True if (i + 1) % interval != 0 else False)
    ...
    def __init__(self,
        ...
        for i in range(stages):
            ...
            Layers.append(
                ...
                norm_layer=self.norm_layer[i], window_size=window_size, use_checkpoint=use_checkpoint, interval=interval)
            )
